...
def test_colony_on_basic_network_structure(
    ...
    assert witness_node.is_running()
    assert colony_node.is_running()
...
def test_colony_on_complex_network(block_log: tt.BlockLog, alternate_chain_spec: tt.BlockLog) -> None:
    ...
    for wn, address in zip(nodes, open_addresses):
        ...
        wn.config.colony_start_at_block = block_log.get_head_block_number() + 10
...
def test_multiple_colony_nodes_communication_with_single_witness_node(
    ...
    for num in range(4):
        ...
        nodes[num].config.colony_start_at_block = block_log.get_head_block_number() + 10
    ...
    assert all(node.is_running() for node in nodes), "Nodes are not running"
    wait_for_start_colony(witness_node, min_trx_in_block=5000)
...
def test_colony_with_unknown_colony_signer(
    ...
    node.config.colony_sign_with.clear()
    node.config.colony_sign_with.append(tt.PrivateKey("unknown-signer-account"))
...
def wait_for_start_colony(node: tt.InitNode | tt.ApiNode, min_trx_in_block: int = 500, timeout: int = 240) -> None:
    ...
    tt.Time.wait_for(is_colony_started, timeout=timeout)
